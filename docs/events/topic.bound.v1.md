...
`PATCH /api/v1/topics/{topicUuid}`가 커밋된 뒤에도 같은 페이로드로 재발행한다. 제목은 이 계약에 없어서 agit는 `topicId` upsert와 `startedAt`만 반영한다. `topic.created`는 수정 시 발행하지 않는다.

## 소비자 기대

- **agit**: 방 읽기모델에 topicId upsert. 없으면 skip
- **chat**: `{주제}가 설정되었습니다` 시스템 메시지. PATCH 재발행 때도 동일하게 수신하므로, 이미 묶인 토픽이면 메시지를 생략할 수 있다.
