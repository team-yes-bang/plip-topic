...
import static com.plip.topic.global.config.InternalApiKeyFilter.HEADER_NAME;
...
class InternalApiKeyFilterTest {
	...
	void allowsInternalRequestWithValidApiKey() throws Exception {
		MockHttpServletRequest request = internalRequest("/internal/v1/videos/uuid/access/user");
	...
	@Test
	void rejectsWhenInternalApiKeyIsNotConfigured() throws Exception {
		filter = new InternalApiKeyFilter(new InternalProperties(""));
		MockHttpServletRequest request = internalRequest("/internal/v1/videos/uuid/access/user");
		request.addHeader(HEADER_NAME, "any-key");
	...
	private MockHttpServletRequest internalRequest(String uri) {
		MockHttpServletRequest request = new MockHttpServletRequest("HEAD", uri);
